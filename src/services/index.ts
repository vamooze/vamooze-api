...
export const services = (app: Application) => {
      ...
      try {
        ...
        let userDetails = await User.find({
          query: {
            $or: [
              { email: req.body.email },
              { phone_number: req?.body?.phoneNumber || 0 },
            ],
          },
        });
        ...
        if (userDetails.data.length < 1) {
          throw new NotFound(
            `${req.body.email ? "Email address" : "Phone number"} not found`
          );
        }
  ...
  app.post(
    ...
    validator.body(schemas.resetPassword),
    async (req: any, res: any) => {
      try {
        let User = app.service("users");
        let userDetails = await User.find({
    ...
    async (req: any, res: any) => {
      try {
        ...
        let result = bcrypt.compareSync(
          ...
          <string>foundUser?.data[0]["password"]
        );
  ...
  app.post(
    "/auth/google/sign-up",
    validator.body(schemas.google_signup),
    async (req: any, res: any) => {
      try {
        let User = app.service("users");
        ...
        const roleData = await app
          .service("roles")
          .find({ query: { $limit: 1, slug: req.body.role } });
  ...
  app.post(
    "/auth/google/sign-in",
    validator.body(schemas.google_signin),
    async (req: any, res: any) => {
      try {
        const data = await app.service("authentication").create({
          password: OAuthTypes.Google,
  ...
  app.post(
    "/resend-otp",
    validator.body(schemas.resend_otp),
  ...
  app.post(
    "/auth/b2b/sign-up",
    validator.body(schemas.b2bSignUp),
    async (req: any, res: any, next: any) => {
      try {
        ...
        const role = await app
          .service("roles")
          .find({ query: { $limit: 1, slug: Roles.B2B } });
        if (role?.data?.length === 0) {
          throw new NotFound("Role not found");
        }
        req.body.role = role?.data[0]?.id;
        ...
        req.body.invited_by = business?.data[0]?.owner
